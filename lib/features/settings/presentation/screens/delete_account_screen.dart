...
class DeleteAccountScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: AppColors.fond, title: const Text('Supprimer le compte')),
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Cette action est irréversible', style: TextStyle(color: AppColors.blanc, fontSize: 22, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            const Text('Votre compte sera désactivé pendant 30 jours avant suppression définitive. Vos données seront effacées et ne pourront pas être récupérées.', style: TextStyle(color: AppColors.gris, fontSize: 15, height: 1.5)),
            ...
            GestureDetector(
              ...
              child: Row(children: [
                ...
                const Expanded(child: Text('Je confirme vouloir supprimer mon compte', style: TextStyle(color: AppColors.blanc, fontSize: 15))),
              ]),
            ),
            const Spacer(),
            SizedBox(width: double.infinity, height: 52, child: ElevatedButton(
              onPressed: s.confirmed && !s.isLoading ? () async {
                try {
                  ...
                  if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Échec de la suppression'), backgroundColor: AppColors.error));
                }
              } : null,
              ...
              child: s.isLoading ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.blanc)) : const Text('Supprimer définitivement', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            )),
            const SizedBox(height: 12),
