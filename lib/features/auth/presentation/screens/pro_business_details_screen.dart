    ...
    return Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.symmetric(horizontal: 24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), color: AppColors.surfaceAlt),
          child: const Text('● CONFIGURATION EN COURS', style: TextStyle(color: AppColors.gris, fontSize: 11, fontWeight: FontWeight.w600))),
        const SizedBox(height: 16),
        const Text('Détails professionnels', style: TextStyle(color: AppColors.blanc, fontSize: 28, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        const Text('Étape 1 : Décrivez vos services pour que les clients puissent vous trouver facilement.', style: TextStyle(color: AppColors.gris, fontSize: 14)),
        const SizedBox(height: 32),
        _field(controller: _businessNameCtrl, label: 'Nom de l\'entreprise', hint: 'ex. Studio Luxe Coiffure', icon: Icons.business),
        const SizedBox(height: 16),
        DropdownButtonFormField<String>(
          initialValue: s.selectedCategory,
          hint: const Text('Sélectionnez votre catégorie principale', style: TextStyle(color: AppColors.gris)),
          ...
          decoration: InputDecoration(labelText: 'Catégorie de service', labelStyle: const TextStyle(color: AppColors.gris), prefixIcon: const Icon(Icons.star_outline, color: AppColors.gris, size: 20), filled: true, fillColor: AppColors.surface, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none)),
        ...
        SizedBox(width: double.infinity, height: 52, child: ElevatedButton(
          ...
          child: s.isLoading ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.fond)) : const Text('Continuer →', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
        )),
        const SizedBox(height: 32),
      ]))),
    );
