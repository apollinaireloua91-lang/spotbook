-- =============================================================================
-- Re-seed quick_reply_templates (idempotent)
-- =============================================================================
-- The original `20260418000000_quick_replies.sql` created the tables and
-- intended to seed default templates, but on the remote DB only the schema
-- landed (the INSERT was either truncated or applied from an earlier shorter
-- version of the file). Result: `quick_reply_templates` is empty.
--
-- This migration inserts the seed data **only if the table is empty**, so it's
-- safe to re-run and a no-op once the templates are in place.
-- =============================================================================

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.quick_reply_templates LIMIT 1) THEN
    RAISE NOTICE 'quick_reply_templates already populated, skipping seed';
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'Seeded quick_reply_templates';
END $$;
